...
standardizeUWTable <- function(tbl)
{
   ...
   tbl.std$siteName <- gsub("#", "", tbl$siteName)
      # Squaring should be, and is now renamed as 'Squaxin'

   deleter <- grep("Squaring", tbl.std$siteName)

   if(length(deleter) > 0)
       tbl.std <- tbl.std[-deleter,]
...
for(f in files){
   ...
   printf("--- reading %s", f)
   tbl <- read.table(f, sep="\t", comment="", quote=NULL, header=TRUE, strip.white=TRUE)
...
mortality <- round(with(tbl, sickOrDeadFernCount/(sickOrDeadFernCount+healthyFernCount)), digits=2)
mortality[which(is.nan(mortality))] <- 0
tbl$mortality <- mortality
tbl$fernCount <- with(tbl, healthyFernCount + sickOrDeadFernCount)
rownames(tbl) <- NULL

# got some temporarily bad data.  remove for now

deleters <- which(is.na(tbl$lat))
if(length(deleters) > 0)
    tbl <- tbl[-deleters,]

westSlopingSites <- which(tbl$aspect >= 180)
eastSlopingSites <- which(tbl$aspect < 180)
stopifnot(length(westSlopingSites) + length(eastSlopingSites) == nrow(tbl))

northerly <- tbl$aspect
northerly[westSlopingSites] <- northerly[westSlopingSites] - 180
northerly[eastSlopingSites] <- 180 - northerly[eastSlopingSites]
tbl$northerly <- northerly

# photos load slowly, and navigate poorly: leave them out for now

deleters <- grep("photos", colnames(tbl))
if(length(deleters) > 0)
    tbl <- tbl[, -deleters]

deadClass.columns <- grep("dead.C", colnames(tbl))
colnames(tbl)[deadClass.columns] <- gsub(".", "", colnames(tbl)[deadClass.columns], fixed=TRUE)
